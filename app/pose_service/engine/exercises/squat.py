...
from ..types import AnalyzeResult, ExerciseBase, Issue
...
class SquatExercise(ExerciseBase):
    ...
    def detect_score(self, series: List[Dict[str, Tuple[float, float]]]) -> float:
        # Heuristik: deutliche Knie-ROM + moderate Torso-Vorlage
        knees: List[float] = []
        torsos: List[float] = []
        for fr in series:
    ...
    def analyze(self, series: List[Dict[str, Tuple[float, float]]]) -> AnalyzeResult:
        knees: List[float] = []
        torsos: List[float] = []
        hip_minus_knee_at_bottom: float | None = None
        max_hip_y = -1.0
        issues: List[Issue] = []
        ...
        if knee_over_toe_flag:
            issues.append(
                Issue(
                    ...
                    msg=(
                        "Knie deutlich vor Zehen → mehr Gewicht auf Fersen, "
                        "Hüfte zuerst zurück."
                    ),
                    severity="warn",
                )
        ...
        if hip_minus_knee_at_bottom is not None and hip_minus_knee_at_bottom <= 0.0:
            issues.append(
                Issue(
                    code="low_depth",
                    msg="Tiefe vermutlich nicht unter parallel.",
                    severity="info",
                )
            )
        ...
        if tmax >= TORSO_LEAN_THRESH:
            issues.append(
                Issue(
                    code="torso_lean_high",
                    msg="Starke Oberkörpervorlage.",
                    severity="info",
                )
            )
        ...
        return AnalyzeResult(
            ...
            issues=issues,
            metrics={
                "knee_min": round(kmin, 1),
                "knee_max": round(kmax, 1),
                "torso_max": round(tmax, 1),
            },
            keyframes=[],
        )
